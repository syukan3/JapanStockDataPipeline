 ...
 * - detectEventsOnDate / detectEventsAll: フィルタ・ページング・session重複の正規化
...
function createChain(result: ChainResult): any {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const chain: any = {};
  const methods = ['select', 'eq', 'neq', 'not', 'order', 'range', 'limit', 'single'];
...
describe('detectEventsOnDate', () => {
  it('factor≠1 の行を数値化して返す（numeric文字列対応）', async () => {
    ...
    const core = createCore([chain]);
    const events = await detectEventsOnDate(core, '2026-06-29');
    expect(events).toEqual([
      { local_code: '31100', trade_date: '2026-06-29', adjustment_factor: 0.2 },
      { local_code: '72360', trade_date: '2026-06-29', adjustment_factor: 0.1 },
    ]);
    expect(chain.eq).toHaveBeenCalledWith('trade_date', '2026-06-29');
  ...
  it('検知0件は空配列', async () => {
    ...
    await expect(detectEventsOnDate(core, '2026-07-14')).resolves.toEqual([]);
  });

  it('クエリエラーは throw', async () => {
    const core = createCore([createChain({ data: null, error: { message: 'timeout' } })]);
    await expect(detectEventsOnDate(core, '2026-07-14')).rejects.toThrow(
      'Failed to detect adjustment events on 2026-07-14: timeout'
    );
  });
});
