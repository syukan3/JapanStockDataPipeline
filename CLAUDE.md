...
注意: Cron の実行主体は GH Actions + pg_cron 外部トリガーのみ。時刻の正は `.github/workflows/`（legacy な Vercel Cron 定義は二重発火のため 2026-07-07 に vercel.json ごと削除済み）。**`vercel.json` は 2026-07-20 に `regions: ["hnd1"]` のみを持つ形で復活させた（東京リージョン固定）。`crons` を書き戻すと二重発火が再発するので絶対に追加しないこと。**
