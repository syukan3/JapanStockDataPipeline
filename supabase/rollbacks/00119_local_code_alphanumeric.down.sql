-- 00119_local_code_alphanumeric.sql の手動ロールバック。
-- 銘柄コードの判定を 00086/00113 時点（数字のみ）へ戻す。
--
-- 注意: 英字入りコード（例: 285A0）の行が残っていると ADD CONSTRAINT の検証が失敗し、
-- このロールバック自体が中断する。先に該当行を削除すること。
...
ALTER TABLE scouter.analyst_rating_history
  ADD CONSTRAINT analyst_rating_history_local_code_check
