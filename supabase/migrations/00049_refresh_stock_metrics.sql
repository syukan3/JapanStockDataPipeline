-- 00049_refresh_stock_metrics.sql
-- analytics.stock_metrics を最新の jquants_core データから再計算する関数。
-- 通期(FY)実績＋最新株価で銘柄指標を算出し、sector17 内の percentile / z-score を付与。
-- Cron A 完了後に `select analytics.refresh_stock_metrics();` で日次実行する想定。
-- パーセンタイルの向き: バリュー=低PER/PBR/PSRほど高スコア、質/モメンタム=高いほど高スコア。
...
begin
  ...
  base as (
    ...
    join fin on fin.local_code = px.local_code
  ),
