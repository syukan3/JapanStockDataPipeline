...
Vercel (Next.js API Routes)
  ...
  ▼
Supabase (PostgreSQL)
  ├─ jquants_core:   7テーブル (ビジネスデータ, RLS保護)
...
GitHub Actions が Vercel API Routes を curl で順次呼び出し。レート制限はトークンバケット方式 (最小間隔 1000ms) で制御。
...
src/
├── app/
│   └── api/cron/jquants/     # Cron API Routes (A, A/chunk, B, C)
├── lib/
│   ├── jquants/
...
├── tests/                    # ユニットテスト (15ファイル, 465ケース)
supabase/migrations/          # DB マイグレーション (12ファイル)
scripts/seed/                 # 初期データ投入 CLI
